...
# --- FUNZIONI CON CACHE (LA PARTE NUOVA) ---
# @st.cache_data dice a Streamlit: "Se hai già caricato questo file, ricordatelo e non rileggerlo da zero!"

@st.cache_data
def load_excel_file(uploaded_file):
    """Legge il file Excel e lo mette in cache"""
...
def load_default_schema():
    """Carica lo schema di default una volta sola"""
...
def main():
    ...
    # Se non c'è il default, chiediamo l'upload manuale
    if df_schema is None:
        # Nota: st.success non va bene qui se il file non c'è, gestiamo diversamente
        uploaded_schema = st.sidebar.file_uploader("Carica Piano Conti FTC", type=['xlsx'])
        if uploaded_schema:
            df_schema = load_excel_file(uploaded_schema) # Cache anche qui
    else:
    ...
    if source_file:
        try:
            # --- LETTURA SOURCE CON CACHE ---
            # Usiamo la funzione ottimizzata invece di pd.read_excel diretto
            df_source = load_excel_file(source_file)
            
            # Necessario fare una copia perché Streamlit non vuole che modifichi i dati in cache
            ...
            if mapping_file:
                try:
                    # Anche qui usiamo la cache
                    df_old = load_excel_file(mapping_file) 
                    ...
                    if k_conto:
                        if 'Cod distinte' in old_cols and 'Descrizione distinte' in old_cols:
                             # Ricreiamo la chiave combinata
                             # (Facciamo la conversione qui al volo perché non possiamo modificare df_old in cache)
                    ...
                    if mappa_dict:
                        # Ottimizzazione: Usiamo map invece di apply riga per riga (molto più veloce)
                        # Convertiamo la colonna conti in stringa per il match
                        conti_series = df_source[c_conto].astype(str)
                        # Mappiamo
                        nuovi_valori = conti_series.map(mappa_dict)
                        # Dove c'è un match e non è "nan", aggiorniamo
